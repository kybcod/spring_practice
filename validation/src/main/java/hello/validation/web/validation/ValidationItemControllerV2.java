...
public class ValidationItemControllerV2 {
    ...
    public String addItemV3(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
        ...
        if (item.getPrice() == null || item.getPrice() < 1000 || item.getPrice() > 1000000) {
            bindingResult.addError(new FieldError("item", "price", item.getPrice(), false, new String[]{"range.item.price"}, null, null));
        }
        if (item.getQuantity() == null || item.getQuantity() >= 9999) {
    ...
    public String addItemV4(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
        ...
        ValidationUtils.rejectIfEmptyOrWhitespace(bindingResult, "itemName", "required"); //ValidationUtils 로 처리

        if (item.getPrice() == null || item.getPrice() < 1000 || item.getPrice() > 1000000) {
            bindingResult.rejectValue("price", "range", new Object[]{1000, 1000000}, null); // 해당 값 넣기
        }
        if (item.getQuantity() == null || item.getQuantity() >= 9999) {
